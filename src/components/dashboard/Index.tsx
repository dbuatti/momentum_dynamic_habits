...
const Index = () => {
  ...
  // NEW: Weekly Goal Habits
  const weeklyGoalHabits = useMemo(() => visibleHabitsForDisplay.filter(h => h.weekly_goal_enabled), [visibleHabitsForDisplay]);
  ...
  useEffect(() => {
    ...
    const initialExpanded = habitGroups.filter(h => {
      ...
      return !h.allCompleted && !h.isLockedByDependency && (h.category === 'anchor' || h.is_trial_mode) && !isWeeklyAnchor && !isWeeklyGoal;
    }).map(h => h.key);
  ...
  const renderHabitItem = (habit: any) => {
    ...
    if (isWeeklyAnchor || isWeeklyGoal) {
      return (
        <WeeklyAnchorCard 
          key={habit.key}
          habit={habit}
      ...
      <div className="max-w-3xl mx-auto w-full px-4 py-6 pb-32">
        ...
        <main className="space-y-8">
          ...
          {weeklyGoalHabits.length > 0 && (
            <div className="space-y-4">
              <div className="sticky top-[60px] z-20 bg-background/95 backdrop-blur-sm py-3 flex items-center gap-3 border-b border-border">
                ...
                <h2 className="text-xs font-black uppercase tracking-[0.2em] text-primary/80">Weekly Goals</h2>
              ...
              <div className="grid gap-4">
                {weeklyGoalHabits.map(habit => (
                  <WeeklyAnchorCard 
                    ...
                    habit={habit}
                    isLocked={habit.isLockedByDependency}
                    dependentHabitName={data.habits.find(h => h.id === habit.dependent_on_habit_id)?.name || 'previous habit'}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            ) : !anchorHabits.length && weeklyGoalHabits.length === 0 ? (
