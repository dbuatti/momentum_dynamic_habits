...
const fetchDashboardData = async (userId: string) => {
    ...
    const profilePromise = supabase.from('profiles').select('journey_start_date, daily_streak, last_active_at, first_name, last_name, timezone, daily_challenge_target, tasks_completed_today').eq('id', userId).single();
    ...
    return {
        ...
        dailyChallengeTarget: profile?.daily_challenge_target || 0,
